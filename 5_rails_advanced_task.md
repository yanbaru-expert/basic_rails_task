...
```zsh:ターミナル
rails new pagination_app -d postgresql -T
cd pagination_app
rails db:create
git add .
git commit -m init
...
- 投稿一覧ページで，投稿の一覧を表示できるようにする
  ...
  - 一覧表示で `each` を使わず「コレクションレンダリング」を利用
  - Railsサーバーを起動して動作確認
...
- レビューが通った後は，プルリクをマージし，mainブランチに移動してプルしてから次の課題に進んで下さい
...
```zsh:ターミナル
rails new recommended_food_app -d postgresql -T
...
- 新規登録・ログイン・ログアウト・アカウント編集機能の動作確認
  ...
  - ログアウトすると, Railsのデフォルト画面が表示されること
...
【注意】課題5のプルリクをマージしていない場合はマージし，mainブランチに移動してプルして下さい。念のため，Rails サーバーを起動し，課題5の機能が反映されていることを確認して下さい。

- ブランチが mainブランチ であることを確認した上で， `task-6` ブランチを作成して切り替える
...
# Rails console を sandbox モードで起動
rails c -s
...
- [【やんばるエキスパート教材】画像投稿機能（ActiveStorage）](https://www.yanbaru-code.com/texts/676)

### 課題7 内容

課題6で作成したアプリに，次の条件で投稿機能に「お勧め料理の画像」を添付できるようにして下さい。

- `ActiveStorage` のマイグレーションファイルを作成して利用できるようにし, Food モデルに画像を追加できるように設定
- 新規投稿・編集フォームから画像も投稿できるように変更
  - 画像ファイル形式のバリデーションを入れること(jpg, gif, png)
  - 画像添付を必須としないこと
  - 教材の「おまけ」の部分を反映するかどうかは自由とする
- 詳細ページで画像が表示されるようにする
  - 一覧ページでは表示させないこと
...
【注意】課題6のプルリクをマージしていない場合はマージし，mainブランチに移動してプルして下さい。念のため，Rails サーバーを起動し，課題6の機能が反映されていることを確認して下さい。

- ブランチが mainブランチ であることを確認した上で， `task-7` ブランチを作成して切り替える

- 「画像投稿機能（ActiveStorage）」教材を参考に進めて下さい。
...
- [【やんばるエキスパート教材】Turbo の基礎](https://www.yanbaru-code.com/texts/677)
...
【注意】課題7のプルリクをマージしていない場合はマージし，mainブランチに移動してプルして下さい。念のため，Rails サーバーを起動し，課題7の機能が反映されていることを確認して下さい。

- ブランチが mainブランチ であることを確認した上で， `task-8` ブランチを作成して切り替える
